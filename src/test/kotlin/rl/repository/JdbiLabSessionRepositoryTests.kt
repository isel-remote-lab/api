...
class JdbiLabSessionRepositoryTests {
    ...
    fun `store lab session, retrieve it and delete it`() {
        repoUtils.runWithHandle { handle ->
            ...
            val labSessionId = labSessionRepo.createLabSession(labId, userId, startTime, endTime)
    ...
    fun `update lab session`() {
        repoUtils.runWithHandle { handle ->
            ...
            labSessionRepo.updateLabSession(labSessionId, newStartTime, newEndTime)
    ...
    fun `store two lab session and retrieve them`() {
        repoUtils.runWithHandle { handle ->
            ...
            val labSessionId1 = labSessionRepo.createLabSession(labId, userId, startTime1, endTime1)

            val startTime2 = startTime1.plus(repoUtils.newTestLabDuration())
            val endTime2 = startTime2.plus(repoUtils.newTestLabDuration())
            val labSessionId2 = labSessionRepo.createLabSession(labId, userId, startTime2, endTime2)
